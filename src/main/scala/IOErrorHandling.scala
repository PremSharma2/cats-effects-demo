...
object IOErrorHandling {
  ...
  val resultAsString: IO[String] = aFailure
    .redeem(
      ex => s"FAIL: $ex",
        value => s"SUCCESS: $value"
    )
  // redeemWith
  val resultAsEffect: IO[Unit] = aFailure
    .redeemWith(
      ex => IO(println(s"FAIL: $ex")),
      value => IO(println(s"SUCCESS: $value")))

  //use case
  def fetchUserDataFromServer(userId: Long): IO[UserData] = IO.delay(UserData(101))

  val userData: IO[UserData] = fetchUserDataFromServer(101)

  val userMessage: IO[String] = userData.redeem(
    ex => s"Error fetching data: $ex",
    data => s"User data: $data"
  )
   ...
   * Exercises
   */
  // 1 - construct potentially failed IOs
  // from standard data types (Option, Try, Either)
